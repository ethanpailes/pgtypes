...
import (
	...
	"github.com/ethanpailes/pgtypes/oid"
)
...
var formatTimeTests = []struct {
	...
	/* TODO(ethan): jackc/pgx doesn't seem to handle times from before the common erra correctly.
	                I think this is something I need to report upstream.
...
TODO(ethan): This is really testing the database driver, and jackc/pgx does not seem
             to parse time strings into golang times. I should report and fix this
			 issue upstream.
...
func TestTimestampWithTimeZone(t *testing.T) {
	...
	for _, locName := range []string{
		...
		for _, pgTimeZone := range []string{"US/Eastern", "Australia/Darwin"} {
			...
			if err != nil {
				t.Logf("Could not load time zone %s - skipping", pgTimeZone)
				continue
			}

			t.Run(fmt.Sprintf("local:%s,pg:%s", locName, pgTimeZone), func(t *testing.T) {
				...
				// NOTE: jackc/pgx seems to return postgres `timestamp with time zone`
				//       in the local time zone of the go process, not the local time zone
				//       of the postgres server as `psql` and lib/pq do. I think the jackc/pgx
				//       way of doing things is actually better for a go driver though.
				gotTime = gotTime.In(pgLoc)
				...
				pgLoc, err := time.LoadLocation(pgTimeZone)
				if err != nil {
					t.Skip(fmt.Sprintf("Could not load time zone %s - skipping", pgLoc))
				}
				translated := refTime.In(pgLoc)

				if translated.String() != gotTime.String() {
					t.Errorf("timestamp strings not equal: %s != %s", translated, gotTime)
				}
			})
		}
	}
...
func TestInfinityTimestamp(t *testing.T) {
	...
	for _, q := range tc {
		...
		if !ok {
			t.Errorf("Scanning -/+infinity, expected string, got %#v", resultI)
		}
		if result != q.ExpectedVal {
			t.Errorf("Scanning -/+infinity, expected %q, got %q", q.ExpectedVal, result)
		}
	}

	/*
	TODO(ethan): I'm not sure if jackc/pgx supports something like EnableInfinityTs.
	             If it does not, I should ask upstream for the feature (I'll likely
				 have to send them patches).
...
func TestBinaryByteSlicetoUUID(t *testing.T) {
	...
	if result != "a0eebc99-9c0b-4ef8-bb00-6bb9bd380a11" {
		t.Fatalf("expected %v but got %v", b, result)
	}
}
